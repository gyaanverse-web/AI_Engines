...
QDRANT_COLLECTION_NAME = os.getenv(
    "QDRANT_COLLECTION_NAME_EXP_03",
    os.getenv("QDRANT_COLLECTION_NAME", "exp_03_openai_ocr_gemini_test"),
)
...
def _get_gemini_sdk():
    try:
        ...
        raise ImportError(
            ...
            "engines/exp_03_openai_ocr_gemini_test/requirements.txt"
        ) from exc
    ...
    for index, step in enumerate(normalized_steps):
        ...
        step_result = _generate_json_response(
            ...
            system_instruction=(
                ...
                "KNOWLEDGE USAGE: "
                "Use only the question, nearby steps, and the provided retrieved context. "
                "Treat the retrieved context as mandatory grounding. "
                "If the retrieved context does not support a claim, mark the step as unknown or incomplete instead of guessing. "
                ...
                "STEP CLASSIFICATION: "
                "step_status must be exactly one of: right, wrong, unknown, incomplete. "
                "Use right for a correct step that advances solution. "
                "Use wrong for incorrect formula, substitution, sign, or clear logic error. "
                "Use incomplete for partial step, setup, missing unit, or unfinished calculation. "
                "Use unknown for OCR or context too unclear to judge. "
                ...
                "FIELDS REQUIREMENT: "
                "topic is an academic topic. "
                "step_understanding is one concise line explaining student intent. "
                "DESCRIPTION RULES: "
                "For wrong steps use exactly 'Error: <exact issue>; Correct step: <correction>'. "
                "For incomplete steps use exactly 'Missing: <missing part>; Correct step: <what should be added>'. "
